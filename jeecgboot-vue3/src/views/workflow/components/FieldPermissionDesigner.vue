<template>
  <div class="field-permission-designer">
    ...
    <a-row :gutter="24">
      ...
      <a-col :span="8">
        <a-card title="流程节点" size="small" class="nodes-panel">
          <div class="search-box">
            <a-input-search v-model:value="nodeSearchKeyword" placeholder="搜索节点..." style="margin-bottom: 16px" @search="filterNodes" />
          </div>
          ...
          <div class="process-nodes">
            ...
            >
              <div class="node-content">
                <div class="node-info">
                  <a-badge :count="getNodeFieldCount(node.id)" :number-style="{ backgroundColor: getNodeBadgeColor(node.id) }" show-zero>
                    <div class="node-icon">
                ...
                <div class="node-status">
                  <a-tooltip :title="getNodeStatusTooltip(node.id)">
                    <a-tag :color="getNodeStatusColor(node.id)" size="small">
      ...
      <!-- 右侧：字段权限配置 -->
      <a-col :span="16">
        <a-card :title="selectedNode ? `${selectedNode.name} - 字段权限配置` : '请选择节点'" size="small" class="permission-panel">
          <template v-if="selectedNode">
            ...
            <div class="field-filters">
              <a-row :gutter="16">
                <a-col :span="12">
                  <a-input-search v-model:value="fieldSearchKeyword" placeholder="搜索字段..." @search="filterFields" />
                </a-col>
                <a-col :span="12">
                  <a-select v-model:value="fieldFilter" placeholder="过滤字段" style="width: 100%" @change="filterFields">
              ...
              <a-row :gutter="16">
                <a-col :span="8">
                  <a-card size="small" title="可编辑字段" class="preview-card editable">
                    <a-space wrap>
                      <a-tag v-for="field in editableFields" :key="field" color="green" size="small">
                        {{ getFieldDisplayName(field) }}
                      </a-tag>
                    </a-space>
                    <div v-if="editableFields.length === 0" class="empty-state"> 暂无可编辑字段 </div>
                  </a-card>
                </a-col>
                <a-col :span="8">
                  <a-card size="small" title="只读字段" class="preview-card readonly">
                    <a-space wrap>
                      <a-tag v-for="field in readonlyFields" :key="field" color="orange" size="small">
                        {{ getFieldDisplayName(field) }}
                      </a-tag>
                    </a-space>
                    <div v-if="readonlyFields.length === 0" class="empty-state"> 暂无只读字段 </div>
                  </a-card>
                </a-col>
                <a-col :span="8">
                  <a-card size="small" title="隐藏字段" class="preview-card hidden">
                    <a-space wrap>
                      <a-tag v-for="field in hiddenFields" :key="field" color="red" size="small">
                        {{ getFieldDisplayName(field) }}
                      </a-tag>
                    </a-space>
                    <div v-if="hiddenFields.length === 0" class="empty-state"> 暂无隐藏字段 </div>
                  </a-card>
                </a-col>
              </a-row>
            </div>
...
<script lang="ts" setup>
  ...
  const props = defineProps({
    ...
    processDefinitionKey: { type: String, required: true },
  ...
  const formFields = ref([
    { fieldName: 'title', fieldType: 'VARCHAR', comment: '标题' },
    { fieldName: 'content', fieldType: 'TEXT', comment: '内容' },
    { fieldName: 'createTime', fieldType: 'DATETIME', comment: '创建时间' },
    { fieldName: 'status', fieldType: 'INTEGER', comment: '状态' },
  ]);
  ...
  const filteredNodes = computed(() => {
    ...
    return processNodes.value.filter(
      (node) =>
  ...
  const filteredFields = computed(() => {
    ...
    if (fieldSearchKeyword.value) {
      ...
      fields = fields.filter(
        (field) => field.fieldName.toLowerCase().includes(keyword) || (field.comment && field.comment.toLowerCase().includes(keyword))
      );
    }
  ...
  const editableFields = computed(() => {
    ...
    return permissions ? permissions.editableFields || [] : [];
  });

  const readonlyFields = computed(() => {
    if (!selectedNode.value) return [];
    const permissions = nodePermissions[selectedNode.value.id];
    return permissions ? permissions.readonlyFields || [] : [];
  });

  const hiddenFields = computed(() => {
    if (!selectedNode.value) return [];
    const permissions = nodePermissions[selectedNode.value.id];
    return permissions ? permissions.hiddenFields || [] : [];
  });
   ...
   * 获取字段权限
   */
  function getFieldPermission(fieldName) {
   ...
   * 更新字段权限
   */
  function updateFieldPermission(fieldName, permission) {
    if (!selectedNode.value) return;
  ...
  // 辅助函数
  function hasNodeConfig(nodeId) {
    const permissions = nodePermissions[nodeId];
    if (!permissions) return false;

    return permissions.editableFields.length > 0 || permissions.readonlyFields.length > 0 || permissions.hiddenFields.length > 0;
  }

  function getNodeFieldCount(nodeId) {
    const permissions = nodePermissions[nodeId];
    if (!permissions) return 0;

    return permissions.editableFields.length + permissions.readonlyFields.length + permissions.hiddenFields.length;
  }

  function getNodeBadgeColor(nodeId) {
  ...
  function getNodeStatusTooltip(nodeId) {
  ...
  function getFieldTypeText(fieldType) {
    const textMap = {
      VARCHAR: '字符串',
      INTEGER: '整数',
      DATETIME: '日期时间',
  ...
  function getFieldDisplayName(fieldName) {
    ...
    return field ? field.comment || fieldName : fieldName;
  }
</script>
    ...
    .nodes-panel {
      ...
      .process-nodes {
        ...
        .node-item {
          ...
          &:hover {
            border-color: #1890ff;
            box-shadow: 0px 2px 4px rgba(24, 144, 255, 0.1);
          }
