  ...
  <div>
        ...
        <a-row :gutter="24">
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="表单ID">
              <a-input placeholder="请输入表单ID" v-model:value="queryParam.cgformHeadId" />
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="流程定义Key">
              <a-input placeholder="请输入流程定义Key" v-model:value="queryParam.processDefinitionKey" />
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="节点ID">
              <a-input placeholder="请输入节点ID" v-model:value="queryParam.nodeId" />
            </a-form-item>
          </a-col>
          <template v-if="toggleSearchStatus">
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="节点名称">
                <a-input placeholder="请输入节点名称" v-model:value="queryParam.nodeName" />
              </a-form-item>
            </a-col>
          </template>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div class="jeecg-basic-table-action-container">
      <div class="jeecg-basic-table-action-left-button">
        ...
        <a-button @click="openPermissionDesigner" type="primary" icon="setting-outlined" style="margin-left: 8px"> 权限配置器 </a-button>
          ...
          <div style="max-width: 200px">
            <a-space direction="vertical" size="small">
              <div v-if="getFieldCount(record.editableFields) > 0">
                <a-tag color="green" size="small"> 可编辑: {{ getFieldCount(record.editableFields) }} </a-tag>
              </div>
              <div v-if="getFieldCount(record.readonlyFields) > 0">
                <a-tag color="orange" size="small"> 只读: {{ getFieldCount(record.readonlyFields) }} </a-tag>
              </div>
              <div v-if="getFieldCount(record.hiddenFields) > 0">
                <a-tag color="red" size="small"> 隐藏: {{ getFieldCount(record.hiddenFields) }} </a-tag>
              </div>
            </a-space>
          </div>
        </template>
    ...
    <!-- 🎯 权限配置模态框 -->
    <WorkflowNodePermissionModal ref="modalRef" @register="registerModal" @success="handleSuccess" />

    <!-- 🎯 权限配置器 -->
    <WorkflowPermissionDesigner ref="designerRef" @success="handleSuccess" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { list, deleteOne, batchDelete, getExportUrl, getImportUrl } from './WorkflowNodePermission.api';
  ...
  const { loading, dataSource, pagination: ipagination, searchQuery, searchReset, handleTableChange, getSelectRows, selectedRowKeys } = tableContext;
